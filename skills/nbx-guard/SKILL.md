...
nbxg plan <type> <id> --set k=v ... | --data '{...}'   创建变更计划（策略 + 风险 + 漂移基线；全部同值 -> no_change 不建计划）
...
**用一整段 JSON 传字段**（`plan` / `create` 通用）：除了逐个 `--set`，还能用 `--data` 传一个
JSON 对象——内联字符串、`@文件`、或 `@-`（stdin）。字段多、或已有一份 JSON 时更省事。
`--data` 与 `--set` 走**同一条**管道（同样的策略 / 风险 / `plan_hash` / 漂移 / 备份 / 审计），
顶层必须是 JSON 对象；可与 `--set` 混用，**从左到右、后者覆盖前者**。

```sh
nbxg create site --data '{"name":"POP3","slug":"pop3"}'      # 内联 JSON
nbxg create site --data @site.json                           # 读文件（或 --data-file site.json）
echo '{"description":"edge router"}' | nbxg plan device 1 --data @-   # 读 stdin
nbxg create site --data @site.json --set status=active       # --data 打底，--set 覆盖
```
